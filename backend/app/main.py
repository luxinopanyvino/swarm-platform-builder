...
# Generated by GitHub Copilot
async def ensure_alexandria_magazine_project() -> None:
    """Ensure the AlexandrIA Magazine system project always exists."""
    async with AsyncSessionLocal() as session:
        ...
        if existing is None:
            project = ProjectModel(
                name="AlexandrIA Magazine",
                description="Proyecto de revista científica con flujo editorial completo.",
                use_case_type=ProjectUseCaseType.ALEXANDRIA_MAGAZINE,
                owner_id=None,
                is_system=True,
            )
            session.add(project)
            ...
            await seed_agents_for_project(project.id, ProjectUseCaseType.ALEXANDRIA_MAGAZINE)
        else:
            # Seed if not yet seeded
            await seed_agents_for_project(existing.id, ProjectUseCaseType.ALEXANDRIA_MAGAZINE)
...
async def lifespan(app: FastAPI):
    ...
    await ensure_alexandria_magazine_project()
    yield
